...
const Community = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      ...
      <div>
        {forums.map(forum => (
          <div key={forum.id} className="bg-white rounded-lg shadow-md mb-6 p-6 transition-transform transform hover:scale-105">
            <h4 className="text-2xl text-gray-800">{forum.title}</h4>
            <p className="text-gray-600 leading-relaxed mt-4">{forum.content}</p>

            {/* Voting System */}
            <div className="flex gap-4 items-center mt-4">
            <button onClick={()=>handleLike(forum._id)} className="bg-gray-100 flex gap-2 hover:bg-gray-200 p-3 rounded-full transition-colors">
                <AiFillLike size={20} color="#2ecc71" />
                <h2>{forum?.upVote>0?forum?.upVote:0}</h2>
              </button>
              <button onClick={()=>handleDislike(forum._id)}  className="bg-gray-100 flex gap-x-2 hover:bg-gray-200 p-3 rounded-full transition-colors">
